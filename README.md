...
**收藏夹只是用户的采集来源（board_name），不是知识库的分类依据。**

- P3 归纳时，AI **只读帖子内容**（title + content）重新生成 `category_l1/l2` 和标签
- 收藏夹名（如"AI"）仅作为 `metadata.board_name` 元信息留存，**不参与分类、不传给 AI 做分类参考**
- 理由：用户收藏习惯与内容主题不一定对应，纯内容分类更准确、更稳定
- 前端展示时：分类筛选用 AI 生成的 `category_l1`；收藏夹名仅作"来源"标签
